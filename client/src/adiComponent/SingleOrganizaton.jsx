...
const SingleOrganization = () => {
  ...
  const fetchOrganization = async () => {
    try {
      const response = await axios.get(`https://crmdemo.vimubds5.a2hosted.com/api/getOrganization/${id}`);
    ...
    <>
      ...
      <div className="w-full max-w-4xl p-10 bg-white rounded-lg shadow-xl">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-3xl font-extrabold text-gray-800">{organization.name || 'No Name Available'}</h2>
          <img
            src={displayLogo}
            alt="Company Logo"
        ...
        <div className="grid grid-cols-1 gap-6 mb-8 md:grid-cols-2">
          ...
          <div>
            <h4 className="text-lg font-semibold text-gray-600">Bank Name:</h4>
            <p className="text-gray-700">{displayBankName}</p>
          </div>
          <div>
            <h4 className="text-lg font-semibold text-gray-600">IFSC Code:</h4>
            <p className="text-gray-700">{displayIfscCode}</p>
          </div>
          <div>
            <h4 className="text-lg font-semibold text-gray-600">Account Number:</h4>
            <p className="text-gray-700">{displayAccNo}</p>
          </div>
          <div>
            <h4 className="text-lg font-semibold text-gray-600">Account Type:</h4>
        ...
        <div className="mb-8">
          <h4 className="text-lg font-semibold text-gray-600">Bank Details:</h4>
          <pre className="p-4 text-gray-800 rounded-lg shadow-inner bg-gray-50">{displayBankDetails}</pre>
        </div>

        <div className="mb-8">
          <h4 className="text-lg font-semibold text-gray-600">Signature:</h4>
          <img src={displaySignature} alt="Signature" className="w-32 h-32 border-2 border-gray-300 rounded-lg shadow-sm" />
        </div>
