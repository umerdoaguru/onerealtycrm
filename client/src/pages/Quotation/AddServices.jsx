...
const Addservices = () => {
  ...
  const getServicelist = async () => {
    try {
      const res = await axios.get(`https://crmdemo.vimubds5.a2hosted.com/api/servicelist/${userId}`);
...
<div className="container mt-5">
  <form className="form-control p-4 bg-white rounded shadow" onSubmit={handleSubmit}>
    <h5 className="mb-4 text-center text-lg font-bold">
      Add Services to Quotation: {quotationName}
      <Link to={`/final-quotation/${id}`} className="btn bg-green-600 text-white mx-3 mb-2 float-end hover:bg-green-700">
    ...
    {services.map((service, index) => (
      <div key={index} className="mb-6">
        <div className="grid gap-4 md:grid-cols-12">
          <h6 className="col-span-12 text-lg font-semibold">Service {index + 1}</h6>
          ...
          <div className="col-span-12 md:col-span-2">
            <label className="block text-sm font-medium text-gray-700">
              ...
              <select
                ...
                required
              >
                <option value="" disabled>Select Subscription Frequency</option>
                {subscriptionFrequencies.map((frequency, key) => (
                  <option key={key} value={frequency}>{frequency}</option>
                ))}
              </select>
            </label>
          </div>
    ...
    <button type="button" className="btn bg-green-600 text-white mt-2 mb-2 hover:bg-green-700" onClick={addService}>
      Add Service
    </button>

    <button type="submit" className="btn bg-green-600 text-white mx-3 mt-2 mb-2 hover:bg-green-700">
      Submit
    </button>
  </form>
</div>
